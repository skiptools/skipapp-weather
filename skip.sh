...
assemble_ipa() {
    ...
    build_ipa() {
        ...
        BUILT_PRODUCTS_DIR=".build" xcodebuild -derivedDataPath .build/DerivedData -skipPackagePluginValidation -archivePath "${ARCHIVE_PATH}" -configuration "${APPCONFIG}" -scheme "App" -sdk "iphoneos" -destination "generic/platform=iOS" -jobs 1 archive CODE_SIGNING_ALLOWED=NO
...
assemble_apk() {
    ...
    build_apk() {
        BUILT_PRODUCTS_DIR=".build" xcodebuild -derivedDataPath .build/DerivedData -skipPackagePluginValidation -configuration ${APPCONFIG} -destination "platform=macosx" -scheme "AppModelKt" -jobs 1 build CODE_SIGNING_ALLOWED=NO
        BUILT_PRODUCTS_DIR=".build" xcodebuild -derivedDataPath .build/DerivedData -skipPackagePluginValidation -configuration ${APPCONFIG} -destination "platform=macosx" -scheme "AppUIKt" -jobs 1 build CODE_SIGNING_ALLOWED=NO
        BUILT_PRODUCTS_DIR=".build" xcodebuild -derivedDataPath .build/DerivedData -skipPackagePluginValidation -configuration ${APPCONFIG} -sdk "macosx" -destination "platform=macosx" -scheme "AppDroid" -jobs 1 build CODE_SIGNING_ALLOWED=NO
